...
0 VALUE NOROOT
: NCDIRI2NAME NOROOT - DIRI2NAME ;
: NCDIRI2TYPE NOROOT - DIRI2TYPE ;

: NCFFNAME. ( DirID n -- )
\  GETXY  NC_NAMESIZE SPACES
\  SETXY
  NCDIRI2NAME ZCOUNT TUCK TYPE \  ZTYPE
...
: NC_SETNAME  ( n -- )
  DUP>R  NC_CUR = 
  DUP	IF DIR_ID NC_CUR NCDIRI2NAME ZCOUNT COUNT_NAME $!
	   DIR_ID NC_CUR NCDIRI2TYPE VFS_TYPE_DIR = TO ?COUNT_DIR
	THEN
 NCSETCOLOR AND
  NC_COLOR  XOR 
 DIR_ID R> NCDIRI2TYPE 5 =  IF 0x4 XOR THEN
 COLOR! ;
...
: DIR_DRAW
 ...
 NC_CUR    0 MAX TO NC_CUR
  ...
  S" "  OPEN-DIR DUP IF S" /" $CHDIR THEN
  THROW TO DIR_ID
  G_CLI_PATH W@ '/' <> 1 AND TO NOROOT
    NC_X_BASE 1+ NC_Y_BASE 1+ SETXY

  NC_CUR DIR_ID DIR2COUNT NOROOT + 1- 0MAX UMIN TO NC_CUR

 NC_COLS NC_ROWS * 0
 DO
	I NC_ROWS /MOD  NC_NAMESIZE 1+ * NC_X_BASE + 1+
	SWAP  NC_Y_BASE + 1+ SETXY 
        I NC_BASE + DIR_ID DIR2COUNT NOROOT + U<
	IF 	I NC_BASE + NC_SETNAME
		DIR_ID I NC_BASE + NCFFNAME.
	ELSE  NC_NAMESIZE SPACES
	THEN
 LOOP
  DIR_ID CLOSE-DIR DROP
;
...
\+ STS.	$13	OF 2>R >R STS. R> 2R> ENDOF
	$F OF 'NC_NCACCEPT_STEP TO NCACCEPT_STEP 
 NCDDDD NCDIR_DRAW
...
\+ STS.	$13	OF 2>R >R STS. R> 2R> ENDOF
	...
	$F OF ['] CO_NCACCEPT_STEP TO NCACCEPT_STEP
                ['] COACCEPT0 TO NCACCEPT  ENDOF \ Ctr-O
	5 OF -1 COUNT_STP +! GET_LAST_STP    ENDOF  \ ctr-e
	$18 OF 1 COUNT_STP +! GET_LAST_STP    ENDOF  \ ctr-x

        $15  OF NC_X_BASE 0x28 XOR TO NC_X_BASE  COLOR@ >R  GETXY 2>R
			FMTABDO FMTABDO   DIR_DRAW	2R> SETXY  R>  COLOR!  ENDOF \ Ctr-U

	6  OF	CUR_DIR@  ACC_TYPE
